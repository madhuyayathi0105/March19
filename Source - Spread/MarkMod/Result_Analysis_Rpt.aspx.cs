...
public partial class Result_Analysis_Rpt : System.Web.UI.Page
{
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        try
        {
                ...
                else
                {
                    ...
                    if (ds_has.Tables.Count > 0 && 0 < ds_has.Tables[0].Rows.Count)
                    {
                        ...
                        for (int i = 0; i < ds_has.Tables[0].Rows.Count; i++)
                        {
                            ...
                            string staff_name = "select staff_name,staff_code from staffmaster where staff_code in (select staff_code from staff_selector where subject_no = " + io + " and batch_year=" + batch_year + "  " + strsec + "); select staff_code from staff_selector where subject_no = " + io + " and batch_year=" + batch_year + "";
                            DataSet dsstf = d2.select_method_wo_parameter(staff_name, "text");
                            if (dsstf.Tables.Count > 0 && dsstf.Tables[1].Rows.Count > 0)
                            {
                                dsv2 = dsstf.Tables[0].Rows.Count;
                                dsv1 = dsstf.Tables[1].Rows.Count;
                                dsv4 = dsstf.Tables[1].Rows.Count;
                            }
                            int dsv3 = dsv1;
                            if (sl_no > 1)
                            {
                            ...
                            for (int seccount = 1; seccount <= dsv1; seccount++)
                            {
                                rowsp++;
                                spancolumns++;
                                drow = data.NewRow();
                            ...
                            for (int sec_temp = 0; sec_temp < dsv; sec_temp++)
                            {
                                ...
                                if (rdexternal.Checked == true)
                                {
                                    ...
                                    string pm = "select count(distinct m.roll_no) from mark_entry m,registration r,subjectChooser sc  where m.roll_no=r.roll_no  " + strsec + "  and m.result = 'Pass' and  m.subject_no =  " + ds_has.Tables[0].Rows[i]["subject_no"] + " and r.delflag<>1 and m.attempts = 1 and ltrim(rtrim(type))='' and m.exam_code=" + examcode_fun + " and sc.subject_no=m.subject_no and m.roll_no=sc.roll_no";   //magesh 17.9.18
                                     passstu = int.Parse(d2.GetFunction(pm));
                                ...
                                if (dsstf.Tables.Count > 0 && dsstf.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    for (int s = 0; s < dsstf.Tables[0].Rows.Count; s++)
                                    {
                                        data.Rows[(data.Rows.Count - dsv3)]["Section"] = secvar.ToString();
                                        if (rbappear.Checked == false)
                                            data.Rows[(data.Rows.Count - dsv3)]["No. Strength"] = tot_stu.ToString();

                                        else
                                            data.Rows[(data.Rows.Count - dsv3)]["No. Appeared"] = tot_stu.ToString();

                                        data.Rows[(data.Rows.Count - dsv3)]["No. passed"] = passstu.ToString();

                                        data.Rows[(data.Rows.Count - dsv3)]["Sectionwise Pass %"] = ddval.ToString();

                                        if (chklstaff.SelectedIndex == 0)
                                        {
                                            data.Rows[(data.Rows.Count - dsv3)]["Subject Teacher"] = Convert.ToString(dsstf.Tables[0].Rows[s]["staff_name"]);
                                        }
                                        if (chklstaff.SelectedIndex == 1)
                                        {
                                            data.Rows[(data.Rows.Count - dsv3) ]["Subject Teacher"] = Convert.ToString(dsstf.Tables[0].Rows[s]["staff_code"]);
                                        }
                                        if (chklstaff.SelectedIndex == 2)
                                        {
                                            data.Rows[(data.Rows.Count - dsv3)]["Subject Teacher"] = Convert.ToString(dsstf.Tables[0].Rows[s]["staff_code"]) + "-" + Convert.ToString(dsstf.Tables[0].Rows[s]["staff_name"]);
                            ...
                            for (int seccount = 0; seccount < dsv4; seccount++)
                            {
                                string ddval = math_tot.ToString();
                                string[] spval = ddval.Split(new char[] { '.' });
                                if (spval.GetUpperBound(0) == 1)
                                ...
                                data.Rows[(data.Rows.Count - dsv4) + (seccount)]["Subjectwise pass %"] = ddval.ToString();
            ...
            {
                string exmcd = string.Empty;
                string subnum1 = string.Empty;
                ...
                else
                {
                    ...
                    if (ds_has.Tables.Count > 0 && 0 < ds_has.Tables[0].Rows.Count)
                    {
                        ...
                        for (int i = 0; i < ds_has.Tables[0].Rows.Count; i++)
                        {
                                    ...
                                    foreach (DataRow row in dsstaff.Tables[0].Rows)
                                    {
                                       ...
                                       // exmcd = string.Empty;
                                        ...
                                        if (ds_exam.Tables.Count > 0 && ds_exam.Tables[0].Rows.Count > 0)
                                        {
                                            ...
                                            string passstudcnt = "select count(marks_obtained) as 'PASS_COUNT',isnull(reg.Stud_Type,'Day Scholar') as Stud_Type,al.sex,reg.mode from result r,registration reg,exam_type e,subjectchooser sc,Applyn al where e.subject_no=sc.subject_no and r.roll_no=sc.roll_no and r.exam_code=e.exam_code and e.exam_code=" + ds_exam.Tables[0].Rows[0]["exam_code"] + " and (marks_obtained>=" + ds_exam.Tables[0].Rows[0]["min_mark"] + " or marks_obtained='-3' or marks_obtained='-2') and reg.roll_no=r.roll_no  and al.app_no=reg.App_No and reg.delflag=0 and reg.exam_flag<>'debar' " + includePassedout + " " + sectionval + " and reg.RollNo_Flag<>0 " + str_staff_qry + "  group by reg.Stud_Type ,al.sex,reg.mode  ";
                                            passstudcnt = passstudcnt + " select count(marks_obtained) as 'Total Appeared' from result r,registration rt,exam_type e,subjectchooser sc where r.roll_no=sc.roll_no and e.subject_no=sc.subject_no and r.exam_code=e.exam_code and r.exam_code=" + ds_exam.Tables[0].Rows[0]["exam_code"] + " and (marks_obtained>=0 or marks_obtained='-2' or marks_obtained='-3' ) and r.roll_no=rt.roll_no and rt.exam_flag <>'DEBAR' " + strincludePassedout + " and rt.cc=0 and rt.RollNo_Flag<>0  and r.roll_no=rt.roll_no  and rt.RollNo_Flag<>0 and rt.batch_year='" + ddlBatch.SelectedItem.ToString() + "' and sc.subject_no='" + ds_exam.Tables[0].Rows[0]["subject_no"].ToString() + "' and rt.degree_code='" + ddlBranch.SelectedValue.ToString() + "' " + sectval + " ";
                                            passstudcnt = passstudcnt + " select count(marks_obtained) as 'OverAll Passcount' from result r,registration reg,exam_type e,subjectchooser sc where e.subject_no=sc.subject_no and r.roll_no=sc.roll_no and r.exam_code=e.exam_code and e.exam_code=" + ds_exam.Tables[0].Rows[0]["exam_code"] + " and (marks_obtained>=" + ds_exam.Tables[0].Rows[0]["min_mark"] + " or marks_obtained='-3' or marks_obtained='-2') and reg.roll_no=r.roll_no and reg.delflag=0 and reg.exam_flag<>'debar' and reg.cc=0  and reg.RollNo_Flag<>0";
                            ...
                            if (dsstdpsct.Tables.Count > 0 && dsstdpsct.Tables[1].Rows.Count > 0)
                            {
                                overallAppear = Convert.ToDouble(dsstdpsct.Tables[1].Rows[0]["Total Appeared"]);
                                if (dsstdpsct.Tables[2].Rows.Count > 0)
                                    overallPass = Convert.ToDouble(dsstdpsct.Tables[2].Rows[0]["OverAll Passcount"]);
                            }
                            ...
                            if (ds_has.Tables.Count > 0 && 0 < ds_has.Tables[0].Rows.Count)
                            {
                                ...
                                for (int sec_temp = 0; sec_temp < dsv; sec_temp++)
                                {
                                    ...
                                    if ((rdinternal.Checked == true) && (in_sec_examcode.ToString() != ""))
                                    {
                                        ...
                                        DataSet ds = new DataSet();
                                        ssd = "select isnull(count(distinct rt.roll_no),0) from result rt,registration r where rt.exam_Code " + in_sec_examcode.ToString() + " and rt.roll_no=r.roll_no and r.degree_code=" + ddlBranch.SelectedValue.ToString() + " and r.batch_year=" + ddlBatch.SelectedItem.ToString() + "  " + strsec + " and (rt.marks_obtained<" + test_minmark + " and rt.marks_obtained<>'-3' and rt.marks_obtained<>'-2' and rt.marks_obtained<>'-18') and r.exam_flag <>'DEBAR' and r.delflag=0 " + includePassedout + " and r.RollNo_Flag<>0  ";
                                        ssd = ssd + "select isnull(count(distinct rt.roll_no),0),isnull(r.Stud_Type,'Day Scholar') as Stud_Type,al.sex,r.mode from result rt,registration r,Applyn al where rt.exam_Code " + in_sec_examcode.ToString() + " and rt.roll_no=r.roll_no and al.app_no=r.App_No and r.degree_code=" + ddlBranch.SelectedValue.ToString() + " and r.batch_year=" + ddlBatch.SelectedItem.ToString() + "  " + strsec + " and (rt.marks_obtained<" + test_minmark + " and rt.marks_obtained<>'-3' and rt.marks_obtained<>'-2' and rt.marks_obtained<>'-18') and r.exam_flag <>'DEBAR' and r.delflag=0 " + includePassedout + " and r.RollNo_Flag<>0  group by r.Stud_Type,al.sex,r.mode ";
                                        ds = d2.select_method_wo_parameter(ssd, "text");
