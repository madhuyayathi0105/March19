...
public partial class Placement_Details : System.Web.UI.Page
{
    ...
    public void fpspread1go1()
    {
        try
        {
            ...
            if (cb_Gender.Checked == true)
            {
                string mulgen = string.Empty;
                for (i = 0; i < cbl_gen.Items.Count; i++)
                {
                    if (cbl_gen.Items[i].Selected == true)
                    {
                        string gen = cbl_gen.Items[i].Value.ToString();
                        if (mulgen == "")
                        {
                            mulgen = gen;
                        }
                ...
                if (addqur == "")
                    addqur = " and a.sex in('" + mulgen + "')";
                else
                {
                    addqur = addqur + " and a.sex in('" + mulgen + "')";
                }
            ...
            else
            {
                ...
                if (chkincluplacement.Checked == true)
                {
                    ...
                    if (cblcom.Items.Count > 0)
                    {
                        ...
                        {
                            if (cblcom.Items[jj].Selected == true)
                            {
                    ...
                    {
                        query = "select  distinct  r.Roll_No,r.Batch_Year,r.Stud_Name,r.degree_code,r.Reg_No,a.Admission_Status,r.Exam_Flag,r.CC,r.Current_Semester,r.DelFlag,a.app_no" + columnname1 + ",case when direct_refer='0'  then 'Direct'   when direct_refer='1'  then 'Staff'   when direct_refer='2'  then 'Others'  when direct_refer='3'  then 'Student' end referby,case when a.direct_refer=1 then (select appl_name from staff_appl_master sa,staffmaster s where sa.appl_no=s.appl_no and convert(varchar(20),s.staff_code)=a.refer_stcode) when direct_refer=3 then (select Stud_Name from applyn where convert(varchar(20),App_No)=a.refer_stcode) when direct_refer=2 then ((select textval from textvaltable where convert(varchar,TextCode)= refer_name)) else '' end refer_stcode,case when direct_refer=2 then ((select textval from textvaltable where convert(varchar,TextCode)= refer_agent)) else '' end as refer_agent from Registration r,applyn a,degree d,Department dt,Course C,Company_StudentREgistration cs,Stud_prev_details s  where  r.App_No=a.app_no and d.Degree_Code =r.degree_code and d.Dept_Code =dt.Dept_Code and c.Course_Id =d.Course_Id and isconfirm ='1' and admission_status ='1' and CC='0' and DelFlag='0' and Exam_Flag<>'DEBAR' and r.degree_code in('" + dept_tagvalue + "') " + addqur + "  " + StateFilter + " " + BoardFilter + " and r.Batch_Year in('" + Batch_tagvalue + "')  and r.App_no=cs.App_no and cs.Shortlist_flag='1' and cs.companyfk in('" + buildvalue + "')";
                    }
                    else
                    {
                        query = "select  distinct  r.Roll_No,cs.com_round,cms.*,r.Batch_Year,r.Stud_Name,r.degree_code,r.Reg_No,a.Admission_Status,r.Exam_Flag,r.CC,r.Current_Semester,r.DelFlag,a.app_no" + columnname1 + ",case when direct_refer='0'  then 'Direct'   when direct_refer='1'  then 'Staff'   when direct_refer='2'  then 'Others'  when direct_refer='3'  then 'Student' end referby,case when a.direct_refer=1 then (select appl_name from staff_appl_master sa,staffmaster s where sa.appl_no=s.appl_no and convert(varchar(20),s.staff_code)=a.refer_stcode) when direct_refer=3 then (select Stud_Name from applyn where convert(varchar(20),App_No)=a.refer_stcode) when direct_refer=2 then ((select textval from textvaltable where convert(varchar,TextCode)= refer_name)) else '' end refer_stcode,case when direct_refer=2 then ((select textval from textvaltable where convert(varchar,TextCode)= refer_agent)) else '' end as refer_agent from Registration r,applyn a,degree d,Department dt,Course C,Company_StudentREgistration cs,Cm_Studentselection cms,Stud_prev_details s  where  r.App_No=a.app_no and d.Degree_Code =r.degree_code and d.Dept_Code =dt.Dept_Code and c.Course_Id =d.Course_Id and isconfirm ='1' and admission_status ='1' and CC='0' and DelFlag='0' and Exam_Flag<>'DEBAR' and r.degree_code in('" + dept_tagvalue + "') " + addqur + "  " + StateFilter + "  " + BoardFilter + " and r.Batch_Year in('" + Batch_tagvalue + "')  and r.App_no=cs.App_no and cs.Shortlist_flag='1' and cms.App_no=cs.App_no and cs.companyfk=cms.companyfk and cs.companyfk in('" + buildvalue + "')";
                    }
                    query = query + "  and  r.Current_Semester in('" + sem_tagvalue + "') " + sectionvalue + " " + FromdateReg + " " + orderStr + "";
                }
                else
                {
                    query = "select  distinct  r.Roll_No,r.Batch_Year,r.Stud_Name,r.degree_code,r.Reg_No,a.Admission_Status,r.Exam_Flag,r.CC,r.Current_Semester,r.DelFlag,a.app_no" + columnname1 + ",case when direct_refer='0'  then 'Direct'   when direct_refer='1'  then 'Staff'   when direct_refer='2'  then 'Others'  when direct_refer='3'  then 'Student' end referby,case when a.direct_refer=1 then (select appl_name from staff_appl_master sa,staffmaster s where sa.appl_no=s.appl_no and convert(varchar(20),s.staff_code)=a.refer_stcode) when direct_refer=3 then (select Stud_Name from applyn where convert(varchar(20),App_No)=a.refer_stcode) when direct_refer=2 then ((select textval from textvaltable where convert(varchar,TextCode)= refer_name)) else '' end refer_stcode,case when direct_refer=2 then ((select textval from textvaltable where convert(varchar,TextCode)= refer_agent)) else '' end as refer_agent from Registration r,applyn a,degree d,Department dt,Course C,Stud_prev_details s  where  r.App_No=a.app_no and d.Degree_Code =r.degree_code and d.Dept_Code =dt.Dept_Code and c.Course_Id =d.Course_Id and isconfirm ='1' and admission_status ='1' and CC='0' and DelFlag='0' and Exam_Flag<>'DEBAR' and r.degree_code in('" + dept_tagvalue + "') " + addqur + "  " + BoardFilter + "  " + StateFilter + " and r.Batch_Year in('" + Batch_tagvalue + "')  ";
                    //string attme = "";
                            ...
                            for (i = 0; i < ds.Tables[0].Rows.Count; i++)
                            {
                                ...
                                if (rdb_cumm.Checked == true)
                                {
                                    ...
                                    if (chkabsent.Checked == false)
                                        arrearcount = d2.GetFunction(" select COUNT (distinct subject_no)as noofarrear from mark_entry where roll_no='" + rollno + "' and subject_no not in(select subject_no from  mark_entry m where  (m.result='pass' or    passorfail='1') and roll_no='" + rollno + "')");
                                    else
                                        arrearcount = d2.GetFunction(" select COUNT (distinct subject_no)as noofarrear from mark_entry where roll_no='" + rollno + "' and subject_no not in(select subject_no from mark_entry m where  (m.result in('pass','AAA')  or    passorfail='1' ) and roll_no='" + rollno + "')");
                                ...
                                else if (rdb_detail.Checked == true)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (removerow != 1)
                                        {
                                            ...
                                            if (chkabsent.Checked == false)
                                                arrcun = d2.GetFunction(" select COUNT (distinct subject_no)as noofarrear from mark_entry where roll_no='" + rollno + "' and subject_no not in(select subject_no from mark_entry m where  (m.result='pass' or    passorfail='1') and roll_no='" + rollno + "')");
                                                //arrcun = d2.GetFunction(" select COUNT (distinct subject_no)as noofarrear from mark_entry where roll_no='" + rollno + "' and subject_no not in(select s.subject_no from subject s,mark_entry m where m.subject_no=s.subject_no and (m.result='pass' or    passorfail='1') and roll_no='" + rollno + "')");
                                            else
                                                arrcun = d2.GetFunction(" select COUNT (distinct subject_no)as noofarrear from mark_entry where roll_no='" + rollno + "' and subject_no not in(select subject_no from   mark_entry m where  (m.result in('pass','AAA') or    passorfail='1') and roll_no='" + rollno + "')");
                                            //string attme = d2.GetFunction("select max(attempts) from mark_entry where roll_no='" + rollno + "' ");
                                            // string attme = ds.Tables[0].Rows[i]["attempts"].ToString();
                                            int att = 0;
                                            ...
                                            {
                                                string excheck = d2.GetFunction("Select top 1 isnull(Exam_Code,-1),exam_month,exam_year from Exam_Details where Degree_Code = '" + dept_tagvalue.ToString() + "' and Current_Semester <= '" + sem_tagvalue + "' and Batch_Year = '" + Batch_tagvalue.ToString() + "' order by Exam_Code desc");
                                                string chkarr = string.Empty;
                                                if (chkabsent.Checked == false)
                                                    chkarr = d2.GetFunction("select COUNT(distinct subject_no) from mark_entry where exam_code='" + excheck + "' and roll_no='" + rollno + "' and  (result<>'Pass' or    passorfail<>'1')");
                                                else
                                                    chkarr = d2.GetFunction("select COUNT(distinct subject_no) from mark_entry where exam_code='" + excheck + "' and roll_no='" + rollno + "' and (result not in('Pass','AAA') or    passorfail<>'1')");
                                                if (att == 1 || chkarr != "0")
                                                {
                                ...
                                else if (Rdb_nil.Checked == true)
                                {
                                    ...
                                    if (arrearcount != "0")
                                    {
                                        ...
                                        if (chkarr == "0")
                                        {
                                            ...
                                            if (removerow != 1)
                                            {
                                                string nilcun = d2.GetFunction("select COUNT(distinct subject_no) from mark_entry  where   roll_no='" + rollno + "' and (result='fail'  or    passorfail='0')");
                                                int arcun = 0;
                                                int.TryParse(nilcun, out arcun);
                                                int txt = 0;
                                                int.TryParse(Txtnil.Text, out txt);
                                                if (arcun == txt)
                                                {
                                ...
                                if (removerow != 1)
                                {
                                    ...
                                    for (int k = 3; k < dtTTDisp.Columns.Count; k++)
                                    {
                                        ...
                                        else if (col == "noofarrear")
                                        {
                                            ...
                                            {
                                                string arrearcount = " select COUNT (distinct subject_no)as noofarrear from mark_entry where roll_no='" + rollno + "' and subject_no not in(select s.subject_no from subject s,mark_entry m where m.subject_no=s.subject_no and (m.result='pass' or    passorfail='1') and roll_no='" + rollno + "')";
                                                nofar.Clear();
