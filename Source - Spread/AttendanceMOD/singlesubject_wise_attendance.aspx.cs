...
public partial class singlesubject_wise_attendance : System.Web.UI.Page
{
    ...
    public void load_student()
    {
        ...
        string minimumabsentsms = d2.GetFunction("select LinkValue from New_InsSettings where LinkName='Studnet Staff Selector' and college_code='" + Session["collegecode"].ToString() + "'");
        ...
        if (splitminimumabsentsms.Length == 2)
        {
            ...
            {
                if (Convert.ToInt32(ddlbatch.SelectedValue.ToString()) >= batchyearsetting)
                {
                    staffSelector = true;
                }
            }
        ...
        if (staffSelector)
        {
            qryStudeStaffSelector = " and sc.staffcode like '%" + Session["Staff_Code"].ToString() + "%'";
            qryStudeStaffSelector1 = " and sc.staffcode=''" + Session["Staff_Code"].ToString() + "''";
        }
