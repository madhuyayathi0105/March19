...
public partial class consolidate_subjwise_attndreport : System.Web.UI.Page
{
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (data.Rows.Count > 0)
            {
                ...
                int temp_col = 0;
                if (data.Columns.Count > 0 && data.Rows.Count > 0 && errmsg.Visible==false)//===========on 9/4/12
                {
        ...
        catch (Exception ex) { d2.sendErrorMail(ex, collegecode, "Consolidate  Subject Wise Report"); }
                ...
                if (no_of_hrs > 0)
                {
                    ...
                    if (subds.Tables[0].Rows.Count != 0)
                    {
                        ...
                        for (int i = 0; i < (subds.Tables[0].Rows.Count); i++)
                        {
                            ...
                            StringBuilder conhr = new StringBuilder("Conducted Periods");
                            AddTableColumn(data, conhr.Append(i));

                            StringBuilder conhr1 = new StringBuilder("Attended Periods");
                            AddTableColumn(data, conhr1.Append(i));

                            StringBuilder conhr2 = new StringBuilder("On Duty Periods");
                            AddTableColumn(data, conhr2.Append(i));

                            StringBuilder conhr3 = new StringBuilder("Absent Periods");
                            AddTableColumn(data, conhr3.Append(i));

                            StringBuilder conhr4 = new StringBuilder("% Att");
                            AddTableColumn(data, conhr4.Append(i));
    ...
    public void load_attendance()
    {
        try
        {
            ...
            ds_alter.Dispose();
            ds_alter.Reset();
            string alterquery = "select  * from alternate_schedule where degree_code = " + ddlbranch.SelectedValue.ToString() + " and semester = " + ddlduration.SelectedItem.ToString() + " and batch_year = " + ddlbatch.SelectedValue.ToString() + "  " + strsec + " order by FromDate Desc";
            ds_alter = da.select_method(alterquery, hat, "Text");//and FromDate ='" + temp_date + "'

            string alterquery1 = "select  * from AlternateDetails where degree_code = " + ddlbranch.SelectedValue.ToString() + " and semester = " + ddlduration.SelectedItem.ToString() + " and batch_year = " + ddlbatch.SelectedValue.ToString() + "  " + strsec + " order by AlternateDate Desc";
            dsalterdet = da.select_method(alterquery1, hat, "Text");//and AlternateDate ='" + temp_date + "'
            //---------------------------------------------
            ...
            {
                for (int colcnt = 0; colcnt <= data.Columns.Count - 1; colcnt++)
                {
                    rcnt = 0;
                    sub_prc = 0;
                    less70perc = 0;
                                ...
                                while (temp_date <= dt2)
                                {
                                    ...
                                    if (!hatdc.Contains(temp_date))//Added by Srinath 5/9/2014 For Day Order Change
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            DataTable dtAlterDet = new DataTable();
                                            if (ds_alter.Tables.Count > 0 && ds_alter.Tables[0].Rows.Count > 0)
                                            {
                                                ds_alter.Tables[0].DefaultView.RowFilter = "FromDate ='" + temp_date + "'";
                                                dtAlter = ds_alter.Tables[0].DefaultView.ToTable();
                                            }
                                            ...
                                            if (dsalterdet.Tables.Count > 0 && dsalterdet.Tables[0].Rows.Count > 0)
                                            {
                                                dsalterdet.Tables[0].DefaultView.RowFilter = "AlternateDate ='" + temp_date + "'";
                                                dtAlterDet = dsalterdet.Tables[0].DefaultView.ToTable();
                                            }
                                            ...
                                            if (ds.Tables[0].Rows.Count > 0)
                                            {
                                                if (no_of_hrs > 0)
                                                {
                                                    ...
                                                    for (int temp_hr = split_holiday_status_1; temp_hr <= split_holiday_status_2; temp_hr++)
                                                    {
                                                        ...
                                                        {
                                                            for (int hasrow = 0; hasrow < dtAlter.Rows.Count; hasrow++)
                                                            {
                                                                full_hour = dtAlter.Rows[hasrow][temp_hr_field].ToString();
                                                                if (full_hour.Trim() != "")
                                                                {
