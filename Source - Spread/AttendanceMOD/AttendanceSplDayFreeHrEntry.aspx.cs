...
public partial class AttendanceSplDayFreeHrEntry : System.Web.UI.Page
{
    ...
    public void Init_Grid()
    {
        try
        {
            ...
            data.Columns.Add("colcode");
            data.Columns.Add("degcode");
            data.Columns.Add("BatchYear");
            data.Columns.Add("Degree");
            data.Columns.Add("Department");
            data.Columns.Add("Semester");
            data.Columns.Add("Section");
            data.Columns.Add("Reason");
            data.Columns.Add("Date");
            data.Columns.Add("Period");
        }
        catch (Exception ex)
        {
            lblErrSearch.Text = Convert.ToString(ex);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(batch_year) && !string.IsNullOrEmpty(collegecode) && !string.IsNullOrEmpty(semester) && !string.IsNullOrEmpty(degree_code) && degselcount != 0)
            {
                ...
                {
                    newqry = "select  r.App_No,r.Roll_No,r.Reg_No,r.Stud_Name,r.Stud_Type,r.degree_code,r.Batch_Year,r.Current_Semester,r.college_code,r.Sections,c.Course_Name,dt.Dept_Name  from Registration r,Course c,Degree dg,Department dt where r.college_code=c.college_code and c.college_code=dg.college_code and dg.college_code=r.college_code and r.college_code=dt.college_code and dt.college_code=c.college_code and dg.Degree_Code=r.degree_code and dg.Dept_Code=dt.Dept_Code and c.Course_Id=dg.Course_Id and r.college_code='" + collegecode + "' and r.Batch_Year='" + batch_year + "' and r.degree_code in(" + degree_code + ") and r.Current_Semester in (" + semester + ")  and isnull(r.sections,'') in(" + section + ") and r.CC='0' and r.DelFlag='0' and Exam_Flag<>'debar'";//ltrim(rtrim(isnull(r.Sections," + section + "))) in('')
                    qry = "select distinct r.degree_code,r.Batch_Year,r.Current_Semester,r.college_code,r.Sections,c.Course_Name,dt.Dept_Name  from Registration r,Course c,Degree dg,Department dt where r.college_code=c.college_code and c.college_code=dg.college_code and dg.college_code=r.college_code and r.college_code=dt.college_code and dt.college_code=c.college_code and dg.Degree_Code=r.degree_code and dg.Dept_Code=dt.Dept_Code and c.Course_Id=dg.Course_Id  and r.college_code='" + collegecode + "' and r.Batch_Year='" + batch_year + "' and r.degree_code in(" + degree_code + ") and r.Current_Semester in (" + semester + ")  and isnull(r.sections,'') in(" + section + ") and r.CC='0' and r.DelFlag='0' and Exam_Flag<>'debar'";//ltrim(rtrim(isnull(r.Sections," + section + "))) in('')
                }
        ...
        try
        {
            ...
            if (collegecode != "" && batch_year != "" && degree_code != "" && semester != "" && degselcount != 0 && secselcount != 0)
            {
                ...
                if (rblSplFree.SelectedValue == "0")
                {
                    if (isfromDate)
                    {
                        qry = "select batch_year,d.degree_code,c.Course_Name,de.Dept_Name,semester,section,Convert(nvarchar(15),entry_date,103) entry_date,period,reason,c.Priority,c.college_code from tbl_spl_attendace a,Degree d,course c,Department de where a.degree_code=d.Degree_Code and d.Course_Id=c.Course_Id and de.Dept_Code=d.Dept_Code and batch_year in(" + batch_year + ") and  d.degree_code in (" + degree_code + ") and semester in (" + semester + ") and isnull(section,'') in(" + section + ")  and entry_date='" + dtFrom.ToString("MM/dd/yyyy") + "' " + selqueries + "  and attype='0'  order by c.college_code,batch_year desc,c.Priority,d.Degree_Code,period,entry_date desc";
                        ...
                        if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                        {
                            ...
                            for (int row = 0; row < ds.Tables[0].Rows.Count; row++)
                            {
                                ...
                                drow = data.NewRow();
                                drow["Sno"] = Convert.ToString(row + 1);
                                drow["colcode"] = coll_code;
                                drow["degcode"] = deg_code;
                                drow["BatchYear"] = batch;
                                drow["Degree"] = coursename;
                                drow["Department"] = dept_name;
                                drow["Semester"] = sem;
                                drow["Section"] = sec;
                                drow["Reason"] = reason.Trim();
                                drow["Date"] = date;
                                drow["Period"] = period;
                                data.Rows.Add(drow);
                ...
                else if (rblSplFree.SelectedValue == "1")
                {
                    if (isfromDate && isToDate)
                    {
                        qry = "select distinct batch_year,d.degree_code,c.Course_Name,de.Dept_Name,semester,section,Convert(nvarchar(15),entry_date,103) entry_date,reason,c.Priority,c.college_code from tbl_spl_attendace a,Degree d,course c,Department de where a.degree_code=d.Degree_Code and d.Course_Id=c.Course_Id and de.Dept_Code=d.Dept_Code and batch_year in(" + batch_year + ") and  d.degree_code in (" + degree_code + ") and semester in (" + semester + ") and isnull(section,'') in(" + section + ") and entry_date>='" + dtFrom.ToString("MM/dd/yyyy") + "' and entry_date<='" + dtTo.ToString("MM/dd/yyyy") + "' and attype='1' " + selqueries + " order by c.college_code,batch_year desc,c.Priority,d.Degree_Code,entry_date desc";
            ...
            if (Showgrid.Rows.Count > 0)
            {
                if (rblSplFree.SelectedValue == "0")
                {
                    foreach (GridViewRow gvrow in Showgrid.Rows)
                    {
                        ...
                        System.Web.UI.WebControls.CheckBox val = (System.Web.UI.WebControls.CheckBox)gvrow.FindControl("selectchk");
                        if (val.Checked == true)
                        {
                            count++;
                            int del = 0;
                            Label colcode = (Label)gvrow.Cells[1].FindControl("lbl_colcode");
                            string coll_code = colcode.Text;
                            Label degcode = (Label)gvrow.Cells[1].FindControl("lbl_degcode");
                            string deg_code = degcode.Text;
                            Label BatchYear = (Label)gvrow.Cells[1].FindControl("lbl_BatchYear");
                            Label Semester = (Label)gvrow.Cells[4].FindControl("lbl_Semester");
                            Label Section = (Label)gvrow.Cells[5].FindControl("lbl_Section");
                            Label Reason = (Label)gvrow.Cells[6].FindControl("lbl_Reason");
                            Label Date = (Label)gvrow.Cells[7].FindControl("lbl_Date");

                            string batch = Convert.ToString(BatchYear.Text);

                            string sem = Convert.ToString(Semester.Text);
                            string sec = Convert.ToString(Section.Text);
                            string reason = Convert.ToString(Reason.Text);
                            ...
                            Label Period = (Label)gvrow.Cells[8].FindControl("lbl_Period");

                            string period = Convert.ToString(Period.Text);
                ...
                else
                {
                    foreach (GridViewRow gvrow in Showgrid.Rows)
                    {
                        ...
                        System.Web.UI.WebControls.CheckBox val1 = (System.Web.UI.WebControls.CheckBox)gvrow.FindControl("selectchk1");
