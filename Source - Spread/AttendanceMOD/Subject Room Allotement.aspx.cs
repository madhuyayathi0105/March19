...
public partial class AttendanceMOD_SubjectRoomAllotement : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (periodDetailsDataSet.Tables.Count > 0 && periodDetailsDataSet.Tables[0].Rows.Count > 0)
            {
                if (Convert.ToString(periodDetailsDataSet.Tables[0].Rows[0]["No_of_hrs_per_day"]) != "")
                {
                    ...
                    inofhours = intNHrs;
                    lblnoofhours.Text = Convert.ToString(inofhours);
                    ...
                    noofdays = nodays;
                    lbldayorder.Text = Convert.ToString(SchOrder);
                    lblnoofdays.Text = Convert.ToString(noofdays);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            int dyodr = 0;
            string nody = lblnoofdays.Text;
            string nohr = lblnoofhours.Text;
            string schd = lbldayorder.Text;
            int.TryParse(schd, out dyodr);
            int.TryParse(nohr, out inofhours);
            int.TryParse(nody, out noofdays);
