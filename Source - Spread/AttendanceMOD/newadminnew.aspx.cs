...
ddlbranch.SelectedValue.ToString() + " and semester=" + ddlsem.SelectedValue.ToString() + "";
    ...
    private void GetFeeOfRollStudent(ref Dictionary<string, DateTime[]> dicFeeOfRollStudents, ref Dictionary<string, byte> dicFeeOnRoll, string fromDate = null, string toDate = null)
    {
        try
        {
            ...
            qry = "select stucon.roll_no,Convert(varchar(50),curr_date,103) as curr_date,Convert(varchar(50),CAST(ack_date as DateTime),103) as ack_date,Convert(varchar(50),feeOnRollDate,103) as feeOnRollDate,semester,ack_fee_of_roll from stucon,registration r where r.roll_no=stucon.roll_no and r.current_semester=stucon.semester and (ack_fee_of_roll=1 or feeOnRollDate is not null) and  CAST(ack_date as DateTime) <='" + dtFromDate.ToString("MM/dd/yyyy") + "' order by CAST(ack_date as DateTime) desc";
